...
def generate_connections(path, filename, ip_addr, flows_dict):
	...
	# pakts = sniff(count=10, socket =  conf.L3socket)

	# print (pakts[0].summary())
	# packets = rdpcap(path + filename)
	# print ("done", flush = True)
	# for k in range(0, min(10,len(packets))):
	# 	print (packets[k].summary())
	# 	print (packets[k].show())
	for (pkt_data, pkt_metadata) in RawPcapReader(path + filename):
		...
		if (((count)%10000) == 0):
			print (count)
			print (cnt_udp, cnt_tcp)


		# ether_pkt = Ether(pkt_data)
		ether_pkt = CookedLinux(pkt_data)
		# print (CookedLinux(pkt_data))
		# print (get_layer_list(ip_pkt))
		# print (ip_pkt.src, ether_pkt[IP].src)
		...
		if ('type' in ether_pkt.fields):
			if (ether_pkt.type == 0x0800):
				...
				assert(ether_pkt.getlayer(IP).src == ip_pkt.src)
				assert(ether_pkt.getlayer(IP).dst == ip_pkt.dst)

		current_time_stamp = ether_pkt.time * MICROSECOND

		# ip_pkt = ether_pkt[IP]
		src_ip = ether_pkt[IP].src
		dst_ip = ether_pkt[IP].dst
	...
	print ("other protocols:", other_protocol)
	print (cnt_udp, cnt_tcp)
...
def has_alpha(pattern):
	chr_set = [chr(ord('a') + i) for i in range(26)]
	pattern = pattern.lower()
	return 1 in [c in pattern for c in chr_set]
			...
			if (filename.lower().find(folder_name) >= 0):
				...
				for line in lines:
					...
					if (line.lower().find(folder_name) >= 0):
						words = line.split()
						for word in words:
							if (word.lower().find(folder_name) >= 0 or has_alpha(word)):
								continue
							ans.append(word)
	res = set(ans)
...
def get_first_time(path, file_name):
	for (pkt_data, pkt_metadata) in RawPcapReader(path + file_name):
		ether_pkt = Ether(pkt_data)
		return ether_pkt.time


def get_list_of_src_flow_obj(feature_dict):
	d = {}

	for key in feature_dict:
		src = feature_dict[key].src 
		if (src not in d):
			d[src] = []
		d[src].append(feature_dict[key])
...
def generate_features_dataset(directory):
	...
	for path in pcap_list:
		...
		list_dir = [filename for filename in list_dir if (filename.find(".csv") < 0)]
		list_dir = natsort.natsorted(list_dir, reverse=False)
		flows_dict = {}
		list_dir = [(get_first_time(path, filename), filename) for filename in list_dir]
		list_dir.sort()
		print (list_dir)
		for first_time,filename in list_dir:
...
generate_features_dataset(root_directory + "Botnet_Detection_Dataset" + os.sep  + "Botnet" + os.sep + "storm" + os.sep)
